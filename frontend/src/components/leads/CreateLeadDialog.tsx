...
export default function CreateLeadDialog({ open, onOpenChange, units, selectedTenantId }: CreateLeadDialogProps) {
  ...
  const createLeadMutation = useMutation({
    ...
    onSuccess: (newLead) => {
      // Invalidate and refetch all related data for real-time sync
      queryClient.invalidateQueries({ queryKey: ['leads'] });
      queryClient.invalidateQueries({ queryKey: ['leads', selectedTenantId] });
      queryClient.invalidateQueries({ queryKey: ['leads', selectedTenantId, 'realtime'] });
      queryClient.invalidateQueries({ queryKey: ['dashboard'] });
      queryClient.invalidateQueries({ queryKey: ['dashboard', selectedTenantId] });
      queryClient.invalidateQueries({ queryKey: ['metrics'] });
      
      // Force immediate refetch of leads and dashboard data
      queryClient.refetchQueries({ queryKey: ['leads', selectedTenantId] });
      queryClient.refetchQueries({ queryKey: ['leads', selectedTenantId, 'realtime'] });
      queryClient.refetchQueries({ queryKey: ['dashboard', selectedTenantId] });
      ...
      queryClient.setQueryData(['dashboard', selectedTenantId], (oldData: any) => {
        if (!oldData) return oldData;
        return {
          ...oldData,
          total_leads: (oldData.total_leads || 0) + 1,
          new_leads: (oldData.new_leads || 0) + 1,
          recent_leads: [newLead, ...(oldData.recent_leads || [])].slice(0, 10)
        };
      });
      
      toast({
        title: 'Sucesso',
        description: `Lead "${newLead.name}" criado com sucesso - Dashboard atualizado`,
      });
      form.reset();
      onOpenChange(false);
    },
