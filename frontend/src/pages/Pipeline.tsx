...
import { LeadsService, UnitsService, useApiStatus } from '@/services/apiService';
...
export default function Pipeline() {
  ...
  // Usa o novo hook de sincronização em tempo real
  const {
    leads,
    isLoading,
    error: leadsError,
    updateLead,
    deleteLead: deleteLeadRealtime,
    forceSync,
    syncStatus,
    refetch
  } = useRealtimeLeads({
    ...
    enablePolling: true,
    onDataUpdate: (updatedLeads) => {
      console.log('📊 Leads atualizados em tempo real:', updatedLeads.length);
    }
  });
  
  // Converte leads para o formato esperado
  const leadsData = {
  ...
  // Mutation para compatibilidade com código existente
  const updateLeadMutation = {
    mutate: updateLead,
    isPending: false // O hook já gerencia o estado de loading
  };

  // Mutation para delete com compatibilidade
  const deleteLeadMutation = {
    mutate: (leadId: string) => {
      deleteLeadRealtime(leadId);
  ...
  const pipelineMetrics = useMemo(() => {
    ...
    return [
        {
            title: 'Novos Hoje',
            value: leadsData.leads.filter(l => isValidDate(l.created_at) && new Date(l.created_at).toISOString().slice(0, 10) === today).length,
            icon: Users,
            color: 'text-blue-400',
        },
        {
